<?
	...
	switch($file_name[0]) {
		...
		case 'index.php':
			...
			$q = "SELECT ID, chrKEY, chrDivision, (SELECT COUNT(S.ID) FROM Stores AS S WHERE S.idDivision=Divisions.ID AND !S.bDeleted) as intCount
				FROM Divisions
				WHERE !bDeleted
			...
			$title = "Administrate Divisions";
			$section = "admin";
			$leftlink = "";
			$page_background = "darkred";
			$headerlink_color = "white";
			// Banner Information
			$banner_title = "Administrate Divisions"; // Title of this page. (REQUIRED)
			$banner_instructions = ""; // Instructions or description. (NOT REQUIRED)
			$page_title = (access_check(7,2) ? linkto(array('address'=>'add.php','img'=>'/images/plus_add.png')):'').' Divisions <span class="resultsShown">(<span id="resultCount">'.mysqli_num_rows($results).'</span> results)</span>';
			$page_instructions = "Select a Division to Edit from the list.";
		...
		case 'add.php':
			...
			if(isset($_POST['chrDivision'])) { include($post_file); }
...
<?
		...
		case 'edit.php':
			...
			if(!isset($_REQUEST['key']) || $_REQUEST['key'] == "") { errorPage('Invalid Division'); } // Check Required Field for Query
			
			$info = db_query("SELECT *
								FROM Divisions
								WHERE !bDeleted AND chrKEY='".$_REQUEST['key']."'","getting Division Info",1); // Get Info
			
			if($info['ID'] == "") { errorPage('Invalid Division'); } // Did we get a result?
...
<?
			...
			$banner_instructions = ""; // Instructions or description. (NOT REQUIRED)
			$page_title = "Edit ".$info['chrDivision'];
