				...
				mcFileManager.open(0, '', '', function(url, info) {
					if (!inst.selection.isCollapsed()) {
						...
						if (tinyMCE.selectElements) {
							...
							nl = tinymce.filter(inst.dom.select('A'), function(n) {return getAttrib(n, 'href') == "javascript:mce_temp_url();";});
	...
	_init3x : function() {
		...
		tinymce.create('tinymce.plugins.FileManagerPlugin', {
			FileManagerPlugin : function(ed, url) {
				...
				ed.addButton('insertfile', 'filemanager_insertfile_desc', 'mceInsertFile', {
