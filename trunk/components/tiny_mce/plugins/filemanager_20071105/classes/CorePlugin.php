<?php
/**
 * $Id: CorePlugin.php 54 2007-06-20 11:57:50Z spocke $
 *
...
class Moxiecode_CorePlugin extends Moxiecode_ManagerPlugin {
	...
	function _listFiles(&$man, $input) {
		...
		$filterRootPath = isset($input["root_path"]) && $input["root_path"] != null ? $man->decryptPath($input["root_path"]) : null;
		...
		if (isset($input["path"]) && $input["path"]) {
			...
			$input["path"] = $man->decryptPath($input["path"]);
