var mcImageManager = {
	...
	open : function(form_name, element_names, file_url, js, s) {
		...
		if (s.iframe) {
			...
			win.contentWindow.document.location = this.getBaseURL() + '/../?type=im';
			return;
		} else
			win = window.open(this.getBaseURL() + '/../?type=im', 'mcImageManagerWin', 'left=' + x + ',top=' + y + ',width=' + w + ',height=' + h + ',scrollbars=yes,resizable=yes,statusbar=no');
